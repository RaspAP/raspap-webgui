function msgShow(retcode, msg) {
  ...
  var htmlMsg =
    ...
    alertType +
    ' alert-dismissible" role="alert"><button type="button" class="btn-close" data-dismiss="alert" data-bs-dismiss="alert" aria-label="Close"></button>' +
    msg +
    "</div>";
  return htmlMsg;
}
...
function setupBtns() {
  $("#btnSummaryRefresh").click(function () {
    getAllInterfaces();
  });
...
$("#hostapdModal").on("shown.bs.modal", function (e) {
  ...
  var countDown = setInterval(function () {
    ...
    document
      ...
      .item(0)
      .setAttribute("style", "width:" + Number(pct) + "%");
    seconds--;
...
$("#install-user-plugin").on("shown.bs.modal", function (e) {
  ...
  if (!installed && repoPublic && installPath === "plugins-available") {
    insidersHTML =
      'Available with <i class="fas fa-heart heart me-1"></i><a href="https://docs.raspap.com/insiders" target="_blank" rel="noopener">Insiders</a>';
  ...
  if (manifestData) {
    ...
    $("#plugin-icon").attr(
      "class",
      `${manifestData.icon || "fas fa-plug"} link-secondary h5 me-2`,
    );
    $("#plugin-name").text(manifestData.name || "Unknown");
    $("#plugin-version").text(manifestData.version || "Unknown");
    $("#plugin-description").text(
      manifestData.description || "No description provided",
    );
    $("#plugin-author").html(
      manifestData.author
        ? manifestData.author +
            (manifestData.author_uri
              ? ` (<a href="${manifestData.author_uri}" target="_blank">profile</a>)`
              : "")
        : "Unknown",
    );
    ...
    $("#plugin-configuration").html(
      formatProperty(manifestData.configuration || "None"),
    );
    $("#plugin-packages").html(formatProperty(manifestData.keys || "None"));
    $("#plugin-dependencies").html(
      formatProperty(manifestData.dependencies || "None"),
    );
    $("#plugin-javascript").html(
      formatProperty(manifestData.javascript || "None"),
    );
    $("#plugin-sudoers").html(formatProperty(manifestData.sudoers || "None"));
    $("#plugin-user-name").html(
      (manifestData.user_nonprivileged &&
        manifestData.user_nonprivileged.name) ||
        "None",
    );
  }
  if (installed) {
  ...
  if (field.is(":input")) {
    ...
    if (!button.data("__toggle-with-initial")) {
      $("i", button)
        ...
        .addClass(button.attr("data-toggle-with"));
    }
