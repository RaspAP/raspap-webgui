...
function _install_dependencies() {
    ...
    if [[ "$php_package" == *"-fpm" ]]; then
        ...
        fpm_output=$(sudo lighty-enable-mod fastcgi-php-fpm 2>&1)
        fpm_status=$?
        echo "$fpm_output"
        if [ $fpm_status -ne 0 ] && [[ "$fpm_output" != *"already enabled"* ]]; then
...
function _patch_system_files() {
    ...
    local switchberry_detected=0
    if [ -x "$webroot_dir/config/switchberry/raspap-switchberryctl" ] && \
       sudo "$webroot_dir/config/switchberry/raspap-switchberryctl" detect >/dev/null 2>&1; then
        switchberry_detected=1
        ...
        sudo visudo -cf "$webroot_dir/config/switchberry/raspap-switchberry.sudoers" || \
            _install_status 1 "Invalid Switchberry sudo rules"
        sudo install -o root -g root -m 0440 \
            "$webroot_dir/config/switchberry/raspap-switchberry.sudoers" \
            /etc/sudoers.d/091_raspap_switchberry || \
            _install_status 1 "Unable to install Switchberry sudo rules"
    fi

    if [ "$switchberry_detected" -eq 1 ] && \
       [ -f /etc/startup-dpll.json ] && [ -f /usr/local/sbin/apply_timing.py ] && \
