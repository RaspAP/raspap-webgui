...
// get all default routes
     exec('ip route list |  sed -rn "s/default via (([0-9]{1,3}\.){3}[0-9]{1,3}).*dev (\w*).*src (([0-9]{1,3}\.){$
     if ( !empty($routes) ) {
        foreach ($routes as $i => $route) {
          ...
          if (isset($checkAccess) && $checkAccess) {
            ...
            exec('ping -W1 -c 1 -I '.$prop[0].' '.ACCESS_CHECK_IP.' |  sed -rn "s/.*icmp_seq=1.*time=.*/\&check\;$
            if (empty($okip)) $okip[0]="failed";
            $rInfo[$i]["access-ip"] = $okip[0];
            exec('ping -W1 -c 1 -I '.$prop[0].' '.ACCESS_CHECK_DNS.' |  sed -rn "s/.*icmp_seq=1.*time=.*/\&check\$
            if (empty($okdns)) $okdns[0]="failed";
            $rInfo[$i]["access-dns"] = $okdns[0];
          }
        }
      } else {
        $data = array("error"=>"No route to the internet found");
      }
